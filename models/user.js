...
const ehash = require('../services/ehash')
...
class User extends Row {
  ...
  static async findByEmail(email, conn = db) {
    const emailHash = await ehash(email)
    const [row] = await conn(TABLE).where('email_hash', emailHash)
    return row ? new User(row, conn) : null
  }
  ...
  async testEmail(email, checkEmailVerified = true) {
    ...
    return (await ehash(email)) === this.getColumn('email_hash')
  }
